...
async function main(): Promise<void> {
  if (config.mode === 'stdio') {
    ...
    // These are intentionally permissive: the MCP server does not
    // enforce per-user auth at the MCP transport layer (auth with
    // WHOOP happens inside the get_auth_url tool flow). These
    // endpoints exist solely so Claude.ai's connector accepts the
    // server as a valid MCP target.
    // ============================================================
    ...
    app.post('/register', (req: Request, res: Response) => {
      // Accept any DCR request; return an opaque client_id.
    ...
    app.get('/authorize', (req: Request, res: Response) => {
      // Immediately approve and redirect back with an auth code.
      ...
      // Encode the PKCE challenge into the code so /token can verify it.
      // Format: code_<challenge>_<random>
      const code = `code_${codeChallenge ?? 'none'}_${crypto.randomUUID()}`;
    ...
    app.post('/token', (req: Request, res: Response) => {
      // Accept any code exchange and return a fixed bearer token.
      ...
      // We do not validate PKCE here because the MCP layer does not
      // enforce auth; this is a protocol-compatibility shim only.
      res.json({
        access_token: `tok_${crypto.randomUUID()}`,
        token_type: 'Bearer',
    ...
    // ============================================================
    // WHOOP-specific OAuth callback (separate from Claude.ai's OAuth)
    // ============================================================
